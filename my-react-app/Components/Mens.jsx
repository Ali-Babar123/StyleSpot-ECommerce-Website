...
const Mens = () => {
  ...
  // Filter the products to only include men's products
  const mensFilteredProducts = filteredProducts.filter(product =>
    product.category === "men's clothing" || product.category === "electronics"
  );
  const productCount = mensFilteredProducts.length;
    ...
    <div>
      ...
      <div className="product-lists">
        {productCount > 0 ? (
          ...
          <p className='m-14 font-medium'>No men's Products Available.</p>
        )}
      </div>
    </div>
  );
